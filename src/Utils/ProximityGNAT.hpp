#ifndef ABETARE__PROXIMITY_GNAT_HPP_
#define ABETARE__PROXIMITY_GNAT_HPP_
...
namespace Abetare
{
	...
	public:
		...
		m_minRadius = HUGE_VAL;
		m_maxRadius =-HUGE_VAL;
		for(int i = 0; i < degree; i++)
		{
		    m_minRange[i] = HUGE_VAL;
		    m_maxRange[i] =-HUGE_VAL;       
		}
	...
	public:
		...
		for(int i = 0; i < m_degree; i++)
		{
		    ...
		    if(m_inner[i]->m_minRadius == HUGE_VAL)
			m_inner[i]->m_minRadius = 0.0;
		    if(m_inner[i]->m_maxRadius == -HUGE_VAL)
		...
		if(degree > 0)
		{
			...
			if(perm[i] >= 0)
			{
			    ...
			    if((r = results->GetMaxDistance()) != HUGE_VAL)
