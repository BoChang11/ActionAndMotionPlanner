#ifndef ABETARE__TRIMESH_HPP_
#define ABETARE__TRIMESH_HPP_
...
namespace Abetare
{
	...
	virtual void AddSideRail(Polygon2D * const poly, 
					const double zmin, const double zmax);
	...
	virtual void AddGrid(const Grid * const g);
	
	
	
	virtual void AddSphere(const double r, const int slices = 20, const int stacks = 20);

	virtual void AddCylinderX(const double r,
				  const double xmin,
				  const double xmax,
				  const int    slices = 20);

	virtual void AddCylinderY(const double r,
				  const double xmin,
				  const double xmax,
				  const int    slices = 20);

	virtual void AddCylinderZ(const double r, 
				  const double zmin, 
				  const double zmax,
				  const int    slices = 20);
	...
	virtual void AddFenceYZ(const double xmin,
				const double ymin,
				const double zmin,
				const double xmax,
				const double ymax,
	...
	virtual void AddHeightMap(const int nrDimsX, const int nrDimsY, const double vertices[]);
		
	virtual void AddHeightField(const HeightField *hf);
 
	virtual void AddRamp(const double width,
			     const double height,
			     const double left,
			     const double top,
			     const double right);
	
	virtual void AddBumpyTiles(const double lengthx, 
	...
	virtual void AddLiftedSpiral(const double lengthx,
				     const double lengthy,
				     const double width,
				     const double height,
				     const double zinc,
	...
	virtual double MaxZIntersect(const double p0[],
				     const double p1[]);
	
	
	virtual void Stitch(Polygon2D * const poly);
